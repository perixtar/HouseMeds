...
const eligible=(includeEstimates:boolean)=>`l.match_status='verified' and l.content_quantity is not null and l.content_unit is not null and s.enabled
...
export function buildApi(pool:pg.Pool,token:string){
 ...
 app.get<{Params:{id:string};Querystring:{quantity?:string;unit?:string;source?:string;location?:string;program?:string;include_estimates?:boolean;limit?:number;cursor?:string}}>('/v1/medications/:id/offers',{schema:{params:{type:'object',required:['id'],properties:{id:{type:'string',pattern:idPattern}}},querystring:{type:'object',additionalProperties:false,properties:{quantity:{type:'string',pattern:'^[0-9]+(?:\\.[0-9]+)?$',maxLength:24},unit:{type:'string',pattern:'^[a-z_]+$',maxLength:32},source:{type:'string',enum:['healthwarehouse','costplus']},location:{type:'string',maxLength:64},program:{type:'string',maxLength:64},include_estimates:{type:'boolean',default:false},limit:{type:'integer',minimum:1,maximum:100,default:50},cursor:{type:'string',pattern:idPattern}}}}},async(req,reply)=>{
  ...
  const {rows}=await pool.query(`select o.id as offer_id,l.id as listing_id,s.slug as source,l.source_name,l.brand_name,l.sold_as,l.content_quantity,l.content_unit,l.url as purchase_url,
   ...
   ${joins} and ${eligible(includeEstimates)} and ${exact} and o.id>$8::bigint order by o.id limit $9`,[...values,query.cursor??'0',limit+1]);
  ...
  return {as_of:asOf,medication,quote_scope:includeEstimates?'including_estimates':'eligible_only',requested_quantity:q,requested_unit:query.unit??null,items:rows.slice(0,limit),next_cursor:more?rows[limit-1].offer_id:null,
