...
test('Lambda Function URL transport preserves preflight, auth errors, and base64 JSON requests', async () => {
  ...
  process.env.HOUSEMED_HOUSEHOLD_SECRET = 'test-only-server-secret-with-at-least-32-characters';
  ...
  const invalid = await handler({rawPath: '/v1/prescription-chat', requestContext: {http: {method: 'POST'}},
    headers: {'x-housemed-household-key':'a'.repeat(64),authorization: 'Bearer '+process.env.HOUSEMED_PRESCRIPTION_API_TOKEN, 'content-type': 'application/json'},
