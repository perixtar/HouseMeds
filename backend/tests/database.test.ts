...
import {approveMedicationBackfill,applyMedicationBackfill,backfillConfirmation,captureBackfillSnapshot,compensateMedicationBackfill,planMedicationBackfill,stageMedicationBackfill} from '../src/backfill.js';
const host=resolve('.cache/pgsock'),port=65431,dbName='housemed_test';
let admin:pg.Pool,worker:pg.Pool,reader:pg.Pool,backfill:pg.Pool,repo:Repository,sourceId:string;let sequence=0;
...
before(async()=>{
 ...
 for(const role of ['housemed_worker','housemed_reader','housemed_backfill','anon','authenticated','service_role'])await bootstrap.query(`drop role if exists ${role}`);
 ...
 await admin.query('alter role housemed_worker login; alter role housemed_reader login; alter role housemed_backfill login');
 worker=new pg.Pool({host,port,database:dbName,user:'housemed_worker',max:2});reader=new pg.Pool({host,port,database:dbName,user:'housemed_reader',max:2});backfill=new pg.Pool({host,port,database:dbName,user:'housemed_backfill',max:2});repo=new Repository(worker);sourceId=(await repo.source('healthwarehouse')).id;
});
after(async()=>{await Promise.all([admin?.end(),worker?.end(),reader?.end(),backfill?.end()]);});
...
test('twelve tables and private access boundaries',async()=>{assert.equal((await admin.query("select count(*)::int as n from information_schema.tables where table_schema='pricing' and table_type='BASE TABLE'")).rows[0].n,12);await assert.rejects(reader.query("update pricing.sources set name='bad'"),{code:'42501'});await assert.rejects(reader.query('select evidence_path from pricing.crawl_runs'),{code:'42501'});await assert.rejects(reader.query('select * from pricing.medication_matches'),{code:'42501'});await assert.rejects(reader.query('select * from pricing.medication_backfill_items'),{code:'42501'});await assert.rejects(worker.query('update pricing.medication_matches set reason=null'),{code:'42501'});await assert.rejects(worker.query("insert into pricing.medication_backfill_events(run_id,event_type,actor) values(1,'staged','bad')"),{code:'42501'});await admin.query('set role anon');try{await assert.rejects(admin.query('select * from pricing.offers'),{code:'42501'});}finally{await admin.query('reset role');}});
...
test('backfill stages, reviews, converges, redirects, reconciles and compensates without price-history loss',async()=>{
 const cp=(await repo.source('costplus')).id;
 const first=(await worker.query("insert into pricing.medications(name,strength,form,route,release_type) values('BACKFILL LISINOPRIL','20MG','tablets','oral','IR') returning id")).rows[0].id;
 const second=(await worker.query("insert into pricing.medications(name,strength,form,route,release_type) values('backfill lisinopril','20 mg','tablet','oral','immediate') returning id")).rows[0].id;
 const unresolved=(await worker.query("insert into pricing.medications(name,strength,form,route,release_type) values('backfill combination','100-25 mg','tablet','oral','immediate') returning id")).rows[0].id;
 const addListing=async(source:string,medication:string,key:string,unit:string,price:string)=>{
  const listing=(await worker.query("insert into pricing.listings(source_id,source_product_key,medication_id,source_name,url,sold_as,content_quantity,content_unit,match_status,metadata) values($1,$2,$3,$4,$5,'pack',1,$6,'verified',$7) returning id",[source,key,medication,'Backfill Lisinopril 20mg Tablets','https://example.test/'+key,unit,JSON.stringify({legacy_evidence:key})])).rows[0].id;
  const run=(await worker.query("insert into pricing.crawl_runs(source_id,parser_version) values($1,'backfill-fixture') returning id",[source])).rows[0].id;
  const offer=(await worker.query("insert into pricing.offers(listing_id,offer_key,quantity,seller_key,location_key,program_key,price_cents,currency,availability,terms,active,last_checked_at,crawl_run_id) values($1,$2,90,$3,'online-us','cash',$4,'USD','in_stock','{}',true,now(),$5) returning id",[listing,'backfill-'+key,key,price,run])).rows[0].id;
  await worker.query("insert into pricing.offer_history(offer_id,crawl_run_id,observed_at,price_cents,currency,availability,snapshot) values($1,$2,now(),$3,'USD','in_stock',$4)",[offer,run,price,JSON.stringify({evidence_path:'local://'+key+'.json'})]);await worker.query("update pricing.crawl_runs set status='succeeded',finished_at=now() where id=$1",[run]);return listing;
 };
 const firstListing=await addListing(sourceId,first,'backfill-hw','tablets','1200'),secondListing=await addListing(cp,second,'backfill-cp','tablet','666');
 const resolver:TerminologyResolver={resolve:async input=>({method:'rxnorm_normalized',version:'fixture-backfill-v1',concepts:input.name.toLowerCase().includes('lisinopril')?[{rxcui:'888314077',name:'backfill lisinopril 20 MG Oral Tablet',tty:'SCD'}]:[]})};
 const normalizer=new MedicationNormalizer(resolver),before=await captureBackfillSnapshot(backfill),plan=await planMedicationBackfill(backfill,normalizer);
 assert.equal((await backfill.query('select count(*)::int n from pricing.medication_backfill_runs')).rows[0].n,0,'dry-run planning must not write');
 const selected=plan.items.filter(item=>[String(first),String(second),String(unresolved)].includes(item.legacy_medication_id));assert.equal(selected.length,3);
 assert.ok(selected.filter(item=>item.decision_status==='ready').every(item=>item.approval_status==='collision_review'&&item.proposed_survivor_id===String(first)));
 assert.equal(selected.find(item=>item.legacy_medication_id===String(unresolved))?.decision_status,'unmatched');
 await assert.rejects(stageMedicationBackfill(backfill,normalizer,{sha256:'a'.repeat(64),created_at:new Date().toISOString(),restore_verified:false,format:'housemed-logical-backup-v3',database_state_hash:'c'.repeat(64)},'database-test'),/RESTORE_VERIFIED_BACKUP_REQUIRED/);
 await assert.rejects(stageMedicationBackfill(backfill,normalizer,{sha256:'a'.repeat(64),created_at:new Date().toISOString(),restore_verified:true,format:'housemed-logical-backup-v3',database_state_hash:'c'.repeat(64)},'database-test'),/BACKUP_DATABASE_STATE_MISMATCH/);
 const staged=await stageMedicationBackfill(backfill,normalizer,{sha256:'a'.repeat(64),created_at:new Date().toISOString(),restore_verified:true,format:'housemed-logical-backup-v3',database_state_hash:plan.baseline.recovery_hash},'database-test');
 await assert.rejects(approveMedicationBackfill(backfill,staged.run_id,'database-test',false),/BACKFILL_HAS_NO_APPROVED_ITEMS/);
 const approval=await approveMedicationBackfill(backfill,staged.run_id,'database-test',true);assert.equal(approval.approved,2);assert.ok(approval.unresolved>=1);
 await assert.rejects(applyMedicationBackfill(backfill,staged.run_id,'database-test','wrong'),/BACKFILL_CONFIRMATION_MISMATCH/);
 assert.equal((await backfill.query('select status from pricing.medication_backfill_runs where id=$1',[staged.run_id])).rows[0].status,'approved');
 const applied=await applyMedicationBackfill(backfill,staged.run_id,'database-test',approval.confirmation);assert.equal(applied.status,'applied');
 const medications=(await worker.query('select id,normalized_key,normalization_status,superseded_by_id from pricing.medications where id=any($1::bigint[]) order by id',[[first,second,unresolved]])).rows;
 assert.deepEqual(medications.map(row=>[row.id,row.normalization_status,row.superseded_by_id]),[[String(first),'verified',null],[String(second),'superseded',String(first)],[String(unresolved),null,null]]);
 assert.equal(medications[0].normalized_key,'rxnorm:888314077');
 const linked=(await worker.query('select id,medication_id,content_unit,metadata from pricing.listings where id=any($1::bigint[]) order by id',[[firstListing,secondListing]])).rows;assert.ok(linked.every(row=>row.medication_id===String(first)&&row.content_unit==='tablet'&&row.metadata.normalization_backfill.run_id===staged.run_id));
 const identity=await apiGet(`/v1/medications/${second}`);assert.equal(identity.statusCode,200);assert.equal(identity.json().requested_medication_id,String(second));assert.equal(identity.json().resolved_medication_id,String(first));
 const redirected=await apiGet(`/v1/medications/${second}/offers?quantity=90&unit=tablet`);assert.equal(redirected.statusCode,200,redirected.body);assert.equal(redirected.json().requested_medication_id,String(second));assert.equal(redirected.json().resolved_medication_id,String(first));assert.equal(redirected.json().items.length,2);
 const status=await apiGet('/v1/normalization/backfill/status');assert.equal(status.statusCode,200);assert.equal(status.json().run.status,'applied');
 const afterApply=await captureBackfillSnapshot(backfill);assert.equal(afterApply.hashes.offers,before.hashes.offers);assert.equal(afterApply.hashes.offer_history,before.hashes.offer_history);assert.equal(afterApply.hashes.listings_immutable,before.hashes.listings_immutable);
 assert.equal((await backfill.query('select count(*)::int n from pricing.medication_matches where backfill_run_id=$1',[staged.run_id])).rows[0].n,2);
 const compensated=await compensateMedicationBackfill(backfill,staged.run_id,'database-test',backfillConfirmation('compensate',staged.run_id,staged.snapshot_hash));assert.equal(compensated.status,'compensated');
 const restored=(await worker.query('select id,normalized_key,normalization_status,superseded_by_id from pricing.medications where id=any($1::bigint[]) order by id',[[first,second,unresolved]])).rows;assert.ok(restored.every(row=>row.normalized_key===null&&row.normalization_status===null&&row.superseded_by_id===null));
 const restoredListings=(await worker.query('select id,medication_id,content_unit,metadata from pricing.listings where id=any($1::bigint[]) order by id',[[firstListing,secondListing]])).rows;assert.deepEqual(restoredListings.map(row=>[row.medication_id,row.content_unit,row.metadata]),[[String(first),'tablets',{legacy_evidence:'backfill-hw'}],[String(second),'tablet',{legacy_evidence:'backfill-cp'}]]);
 const afterCompensation=await captureBackfillSnapshot(backfill);assert.equal(afterCompensation.hashes.medications,before.hashes.medications);assert.equal(afterCompensation.hashes.components,before.hashes.components);assert.equal(afterCompensation.hashes.listings,before.hashes.listings);assert.equal(afterCompensation.hashes.offers,before.hashes.offers);assert.equal(afterCompensation.hashes.offer_history,before.hashes.offer_history);
 assert.equal((await backfill.query('select count(*)::int n from pricing.medication_matches where backfill_run_id=$1',[staged.run_id])).rows[0].n,2,'append-only match evidence survives compensation');
});
...
test('catalog labels are literal search data and SQL injection cannot broaden results',async()=>{
 ...
 assert.equal((await worker.query('select count(*)::int n from pricing.sources')).rows[0].n,3);
});
