...
export const prescriptionInput=z.object({action:z.enum(['chat','confirm','confirm_all','prepare','create_member']),request_id:z.string().uuid(),message:z.string().max(4000).default(''),image:photo.optional(),draft_id:z.string().uuid().optional(),member_id:z.string().uuid().optional(),fields:fields.optional(),nickname:field.min(1).max(80).optional(),pending_action:z.literal('confirm_all').nullable().optional(),reviewed_drafts:z.array(z.object({draft_id:z.string().uuid(),fields}).strict()).max(40).optional()}).strict().refine(value=>value.action!=='create_member'||Boolean(value.nickname),'Member nickname is required');
