...
if(process.platform!=='darwin'||!['install','restart','status','uninstall'].includes(action)||!['api','backup','pilot','all'].includes(selected))throw Error('Usage: node scripts/mac-services.mjs install|restart|status|uninstall api|backup|pilot|all (macOS only)');
...
for(const name of selected==='all'?['api','backup','pilot']:[selected]){
 const label='com.housemed.'+name;const path=resolve(directory,label+'.plist');
 const service={Label:label,WorkingDirectory:root,ProgramArguments:[process.execPath,...(name==='api'?['--import','tsx',resolve('src/api.ts')]:name==='pilot'?['--import','tsx',resolve('src/schedule.ts'),'tick']:[resolve('scripts/backup.mjs')])],StandardOutPath:resolve(`data/logs/${name}.log`),StandardErrorPath:resolve(`data/logs/${name}.error.log`),Umask:63,EnvironmentVariables:{PATH:process.env.PATH},...(name==='api'?{RunAtLoad:true,KeepAlive:true,ThrottleInterval:30}:name==='pilot'?{RunAtLoad:true,StartInterval:300}:{StartCalendarInterval:{Hour:3,Minute:15}})};
